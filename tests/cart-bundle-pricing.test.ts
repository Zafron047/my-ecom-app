...
describe('computeCartPricing bundle tiers', () => {
  ...
  it('treats empty eligible variants as all variants and picks best eligible tier', () => {
    const lines: CartLineInput[] = [
    ...
    const pricing = computeCartPricing(lines, getOffersByProduct(offers));
    expect(pricing.linePricingById.l1.bundleDiscountPercent).toBe(20);
    expect(pricing.linePricingById.l2.bundleDiscountPercent).toBe(20);
    expect(pricing.discountTotal).toBe(100);
  });
  ...
  it('applies next best offer on remaining variants after a variant-specific winner', () => {
    ...
    const offers: BundleOfferLite[] = [
      { id: 'o30', title: 'Buy 10 Bags get 30% OFF', minTotalQty: 10, discountPercent: 30, variantIds: [], isActive: true },
      { id: 'o20', title: 'Buy 5 Bags get 20% OFF', minTotalQty: 5, discountPercent: 20, variantIds: [], isActive: true },
      { id: 'o10', title: 'Buy 3 Bags get 10% OFF', minTotalQty: 3, discountPercent: 10, variantIds: [], isActive: true },
  ...
  it('chooses global max discount over greedy local winner', () => {
    ...
    const offers: BundleOfferLite[] = [
      ...
      { id: 'all35', title: 'All 35%', minTotalQty: 10, discountPercent: 35, variantIds: [], isActive: true },
    ];
